...
namespace UseCases.Catalogs.Properties.GetPagedList;
public partial class GetPropertyPagedList
{
    public sealed record Param : QueryParams;
    public sealed record Result : PropertyListItemResult;
    ...
    ) : IQueryHandler<Query, PagedList<Result>>
    {
        public async Task<ErrorOr<PagedList<Result>>> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            catch (Exception ex)
            {
                ...
                return Property.Errors.PropertyUnexpected(nameof(GetPropertyPagedList), ex.Message);
            }
        }
    }
}
