...
namespace UseCases.Admin.Catalogs.OptionTypes.GetOptionList;
public partial class GetOptionTypeOptionList
{
    public sealed record Param : QueryParams;
    public sealed record Result : OptionTypeSelectItemResult;
    ...
    ) : IQueryHandler<Query, PagedList<Result>>
    {
        public async Task<ErrorOr<PagedList<Result>>> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            catch (Exception ex)
            {
                ...
                return OptionType.Errors.OptionTypeUnexpected(nameof(GetOptionTypeOptionList), ex.Message);
            }
        }
    }
}
